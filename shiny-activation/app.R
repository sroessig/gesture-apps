...
solve_gesture <- function(max_t, k, ae) {
  # time step
  dt <- 0.01
...
if (generate_data) {
  df <- data.frame()
  for (ae in seq(ae_min, ae_max, ae_step)) {
    ae <- round(ae, 1)
    G <- solve_gesture(max_t, 6, ae)
    df <- rbind(df, data.frame(ae, G))
  }
...
ui <- fluidPage(titlePanel(""),
                ...
                fluidRow(
                  column(4,
                         wellPanel(
                           h4("Activation end of red opening gesture", style = "color: #cc0000"),
                           sliderInput(
                             inputId = "ae",
                             label = "",
                             min = ae_min,
...
server <- function(input, output) {
  output$evoPlot <- renderPlot({
    ...
    rect(xleft = ae1, xright = ae1+300, ybottom = 0, ytop = 1, col = rgb(0.8,0,0,0.3), border = red)
    text(x = ae1+300/2, y = 0.5, labels = "labial closure")
    ...
    rect(xleft = ae2, xright = ae2+300, ybottom = 1.1, ytop = 2.1, col = "lightgrey", border = "dimgrey")
    text(x = ae2+300/2, y = 1.6, labels = "labial closure")
    title(main = "gestural score (activations)", line = 0.5, adj = 0)
    
    plot(d1$TM, d1$X, type = "n", xlab = "", ylab = "", ylim = c(-1.0, 0.1), yaxt = "n")
    ...
    text(x = max_t-25, y = -0.97, labels = "open")
